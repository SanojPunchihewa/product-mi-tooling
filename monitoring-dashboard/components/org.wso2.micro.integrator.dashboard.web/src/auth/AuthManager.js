...
export default class AuthManager {
     ...
     */
    static authenticate(username, password, rememberMe, isSecure = true) {
        ...
        return new Promise((resolve, reject) => {
            if (username == "admin") {
                ...
                AuthManager.setUser({username: username}, isSecure);
                // secure token cookie will only be set in https mode
                AuthManager.setCookie(Constants.JWT_TOKEN_COOKIE, "abc@123", 3600, window.contextPath, isSecure);
                resolve();
            } else {
